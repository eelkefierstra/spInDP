  ...
  (JNIEnv *env, jobject, jstring port)
{
	...
	if ((serialPort = open(device, O_RDWR | O_NOCTTY | O_SYNC)) < 0)
	{
	...
	set_interface_attribs(serialPort, B1000000, 0);
	set_blocking(serialPort, 0);
}
  ...
  (JNIEnv *env, jobject)
{
	...
	int len = 0;
	if ((len = read(serialPort, buff, sizeof buff)) < 0)
	{
		close(serialPort);
		serialPort = 0;
		string clazz = "java/io/IOException";
